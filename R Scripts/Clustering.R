...
# LeBron is group 1 for AssignedGroup3, 5 for AssignedGroup5
...
# 
colnames(cluster.df) <- c('Games Played','Minutes / Game','Start %','Field Goal %',
                          'Two Point %',' Two Points / Minute','Three Point %',
                          'Three Points / Minute','Points / Minute','Assists / Minute',
                          'Rebounds / Minute','Free Throws / Minute',
                          'Fouls / Minute','AssignedCluster3','AssignedCluster5')


cluster.df %>% 
  select(-c(AssignedCluster5)) %>% 
  group_by(AssignedCluster3) %>% 
  summarise_all(median) %>% 
  pivot_longer(-c(AssignedCluster3)) %>% 
  ggplot(mapping = aes(AssignedCluster3, name)) +
  ...
  geom_text(aes(x = AssignedCluster3, y=name
...
cluster.df %>% 
  select(AssignedCluster3) %>% 
  write.csv('Cluster_Assignments.csv', row.names = T)
